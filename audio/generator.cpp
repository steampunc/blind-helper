...
int AudioGenerator::Callback(const void *input, void *output,
  ...
  for (uint64_t i = 0; i < frameCount; i++, (data->counter)++) {
    ...
    double user_sound_range_constant = 1.01457;
    double user_sound_base_constant = 100;

    if (vision_input) {
      for (int j = 0; j < vision_input.value()->section_size(); j++) {
        double left_pitch =
            std::pow(user_sound_range_constant,
                     double(vision_input.value()->section(j).a())) * user_sound_base_constant;
        double right_pitch =
            std::pow(user_sound_range_constant,
                     double(vision_input.value()->section(j).a())) * user_sound_base_constant;
...
void AudioGenerator::EvalError(PaError error) {
  if (error != paNoError)
    std::cout << "PortAudio error: " + error << std::endl;
}
}
}
